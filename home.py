...
def basicPageSetup(pageNo):
    ...
    back_img = tk.PhotoImage(file=r"D:\Criminal detection\img\back.png")
    ...
    heading = tk.Label(
        pages[pageNo], fg="white", bg="#3E3B3C", font="Arial 20 bold", pady=10
    )
    heading.pack()
...
def getNewSlide(control):
    ...
    if len(img_list) > 1:
        ...
        slide_caption.configure(
            text="Image {} of {}".format(current_slide + 1, len(img_list))
        )
...
def selectMultiImage(opt_menu, menu_var):
    ...
    path_list = filedialog.askopenfilenames(
        title="Choose atleast 5 images", filetypes=filetype
    )
...
def register(entries, required, menu_var):
    ...
    for i, entry in enumerate(entries):
        ...
        if len(val) == 0 and required[i] == 1:
            messagebox.showerror(
                "Field Error", "Required field missing :\n\n%s" % (entry[0])
            )
            return
        else:
    ...
    if len(no_face) > 0:
        ...
        if rowId >= 0:
            ...
            cv2.imwrite(
                "profile_pics/criminal %d.png" % rowId, img_list[profile_img_num]
            )
            ...
            shutil.rmtree(path, ignore_errors=True)
            messagebox.showerror(
                "Database Error", "Some error occured while storing data."
            )
...
def getPage1():
    ...
    canvas.bind(
        "<Configure>",
        lambda event, canvas=canvas, win=scroll_win: on_configure(event, canvas, win),
    )
    ...
    for i, field in enumerate(input_fields):
        ...
        if i != ip_len - 1:
            ...
            menu.configure(
                font="Arial 13", bg="white", activebackground="#90ceff", bd=0
            )
...
def showCriminalProfile(name):
    ...
    for i, item in enumerate(crim_data.items()):
        ...
        tk.Label(
            info_frame, text=val.capitalize(), fg="white", font="Arial 15", bg="#202d42"
...
def startRecognition():
    ...
    if len(face_coords) == 0:
        messagebox.showerror(
            "Error", "Image doesn't contain any face or face is too small."
        )
    else:
        (model, names) = train_model()
        print("Training Successful. Detecting Faces")
        (frame, recognized) = recognize_face(
            model, frame, gray_frame, face_coords, names
        )
        ...
        for i, crim in enumerate(recognized):
            ...
            crims_found_labels[i].bind(
                "<Button-1>", lambda e, name=crim[0]: showCriminalProfile(name)
            )
...
tk.Label(
    pages[0],
    text="Face Recognition System for Criminal Detection",
    fg="black",
    bg="#3E3B3C",
    font="Arial 25 bold",
    pady=30,
